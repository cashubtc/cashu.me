...
const amountPerInterval = computed(() => {
  ...
  if (!s) return "";
  const periodAmount =
    s.receivedPeriods > 0 ? s.totalAmount / s.receivedPeriods : s.totalAmount;
...
const payments = ref<
  Array<{ id: string; status: string; date: string; amount: number }>
>([]);
