    ...
    <div class="text-center" style="max-width: 800px; width: 100%">
      ...
      >
        <template v-slot:append>
          <q-btn flat dense icon="content_paste" class="cursor-pointer q-mt-md" @click="paste" />
        </template>
      </q-input>
      <p class="q-mt-none" style="font-size: 0.8rem; color: grey">We use this only locally to derive your wallet keys.</p>
    </div>
  </div>
</template>
...
export default {
  ...
  setup() {
    ...
    const errorMsg = computed(() => {
      ...
      return words.length >= 12 ? '' : 'Mnemonic should be at least 12 words.';
    });

    watch(() => restore.mnemonicToRestore, (val) => {
...
<style scoped>
.seed-phrase :deep(.q-field__control) { padding: 12px 12px !important; }
.seed-phrase { font-size: 0.9rem; font-family: monospace; }
h2 { font-weight: bold; }
p { font-size: large; }
</style>
