...
function main() {
  const entries = fs.readdirSync(i18nDir, { withFileTypes: true })
    ...
    .map((d) => ({ code: d.name, file: path.join(i18nDir, d.name, 'index.ts') }))
  ...
  const locales = entries.map((e) => ({ code: e.code, data: loadLocale(e.file) }));
  const flatMaps = locales.map(({ code, data }) => ({ code, flat: flatten(data) }));
