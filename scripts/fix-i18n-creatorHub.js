...
for (const file of files) {
  ...
  // find every "creatorHub: {" occurrence
  const hits = [];
  ...
  // remove all but the FIRST creatorHub block
  const removals = [];
  for (let i = 1; i < hits.length; i++) {
    ...
    // eat trailing whitespace and a trailing comma (to keep valid syntax)
  ...
  removals.sort((a,b) => b[0]-a[0]).forEach(([from,to]) => {
