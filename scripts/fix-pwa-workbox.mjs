...
const files = ['quasar.config.js', 'quasar.config.ts'].filter(f => fs.existsSync(f));
if (!files.length) {
  console.error('Could not find quasar.config.js or quasar.config.ts in the repo root.');
  process.exit(1);
}
...
for (const file of files) {
  ...
  s = s.replace(/workboxMode\s*:\s*['"]generateSW['"]/gi, "workboxMode: 'GenerateSW'");
  s = s.replace(/workboxMode\s*:\s*['"]injectManifest['"]/gi, "workboxMode: 'InjectManifest'");
